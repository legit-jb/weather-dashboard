...
// expects num = the temp of uv index in a number, the id if the element to change, and wether it is uv or temp
...
checkWeather = (checkCity) => {
    ...
    fetch(requestUrl)
        ...
        .then(data => {
            ...
            todaysCondition.setAttribute("src", `http://openweathermap.org/img/wn/${weather[0].icon}@4x.png`);
...
const getFiveDay = (daily) => {
    ...
    for (let i = 1; i < 6; i++) {
        ...
        fiveConditionEl.setAttribute("src", `http://openweathermap.org/img/wn/${weather[0].icon}@2x.png`);
        fiveConditionEl.setAttribute("aria", weather[0].description);

        const fiveTempEl = document.getElementById(`day${i}-temp`);
        const fiveTemp = Math.round(((temp.min + temp.max / 2) + Number.EPSILON) * 100) / 100;
